...
const WidgetUploadDocument = (props) => {
  ...
  const { idCustomerOwner = null, dataDetail } = dataContexProperty;
  ...
  const handlerCallDeactivateCustomerDocument = async (data, id) => {
    ...
    try {
      const response = await callGlobalActionApi(
        ...
        id,
        API_CONSTANTS.CUSTOMER.DEACTIVATE_PROPERTY_DOCUMENT,
        "PUT"
      );
  ...
  const handlerCallAddCustomerDocument = async (data, id) => {
    ...
    try {
      const response = await callGlobalActionApi(
        ...
        id,
        API_CONSTANTS.CUSTOMER.ADD_PROPERTY_DOCUMENT,
        "PUT"
      );
      const responseResult =
        isNil(response.response) === false &&
                        ...
                        <>
                          <label className="upload-file" for={`id-file-${ix}`}>
                            <IconActivity
                              ...
                              stroke="#A0A3BD"
                            />
                            <span>Has clic para subir archivo</span>
                          </label>
                          <input
                            id={`id-file-${ix}`}
                            accept="image/*,.pdf,.docx"
